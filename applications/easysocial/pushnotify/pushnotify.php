...
class SocialUserAppPushnotify extends SocialAppItem
{
	...
	public function onNotificationBeforeCreate( $item_dt)
	{
		...
		//$participants = $alert->getUsers('system',$participants);
		...
		$options = array(
						'unread' => true,
						'target' => array('id' => $targetUsers[0], 'type' => SOCIAL_TYPE_USER)
					);						
		...
		$msg_data['mssge']= (isset($msg_data['mssge']))?strip_tags($msg_data['mssge']):'';
